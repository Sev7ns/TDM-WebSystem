...
import { 
  ...
  ArrowRight, Palette, ExternalLink, Layers, Ticket, Landmark as Bank, ListFilter, SlidersHorizontal, Settings2, Box
} from 'lucide-react';
import { Transaction, User, TransactionStatus, SystemConfig, CostGroup, AdminLevel, CurrencyConfig, PaymentMethodConfig, Coupon, ScheduleConfig, ServiceItem, BudgetTheme, ReferenceItem, SocialLink, CostItem, GatewayProfile } from '../types';
...
const InternalSimulator = ({ config, activeGatewaySlug }: { config: SystemConfig, activeGatewaySlug: string }) => {
   const [simAmount, setSimAmount] = useState(100);
   const [couponCode, setCouponCode] = useState('');
   const [currency, setCurrency] = useState('VES');
   const [result, setResult] = useState<any>(null);

   const activeGateway = config.logistics.gatewayProfiles.find(g => g.slug === activeGatewaySlug) || config.logistics.gatewayProfiles[0];

   useEffect(() => {
      setResult(MockBackend.calculateQuote(simAmount, currency, undefined, couponCode, activeGatewaySlug));
   }, [simAmount, couponCode, currency, config, activeGatewaySlug]);
      ...
      <div className="bg-slate-900 text-white p-6 rounded-2xl shadow-xl border border-slate-700 h-fit sticky top-6">
         <div className="flex justify-between items-center mb-6 border-b border-slate-700 pb-4">
            <h4 className="text-xs font-bold text-brand-400 uppercase tracking-widest flex items-center gap-2">
                <Activity size={14}/> Simulador ({activeGateway.label})
            </h4>
            ...
            <div className="flex gap-3">
               <div className="flex-1">
                   ...
                   <select value={currency} onChange={e => setCurrency(e.target.value)} className="w-full bg-slate-800 border border-slate-600 rounded-lg p-2 text-white text-sm focus:ring-1 focus:ring-brand-500 outline-none">
                       {activeGateway.currencies.filter(c => c.enabled).map(c => <option key={c.code} value={c.code}>{c.code}</option>)}
                   </select>
               </div>
               <div className="flex-1">
             ...
             <div className="bg-slate-800/50 p-3 rounded-xl border border-slate-700/50">
                ...
                <div className="relative pl-4 border-l-2 border-slate-700 ml-1 py-1 space-y-1">
                   <div className="flex justify-between text-red-400">
                      ...
                      <span>-${(result.opsDeductions || 0).toFixed(2)}</span>
                   </div>
                </div>
                <div className="flex justify-between font-bold text-white border-t border-slate-600 pt-2 mt-2">
                   <span>Base para Cliente</span>
                   <span className="text-blue-300">${(result.baseForExchange || 0).toFixed(2)}</span>
                </div>
             </div>

             <div className="relative pl-4 border-l-2 border-slate-700 ml-1 py-1 space-y-1">
                {/* Costos Absorbidos (Admin) */}
...
const LogisticsGateways = ({ config, onUpdate, activeGatewaySlug }: { config: SystemConfig, onUpdate: (c: Partial<SystemConfig>) => void, activeGatewaySlug: string }) => {
   const [expandedIndex, setExpandedIndex] = useState<number | null>(null);
   const gatewayIndex = config.logistics.gatewayProfiles.findIndex(g => g.slug === activeGatewaySlug);
   const gateway = config.logistics.gatewayProfiles[gatewayIndex];

   if(!gateway) return <div>Seleccione una pasarela</div>;

   const updateCurrencies = (newCurrencies: CurrencyConfig[]) => {
       const newProfiles = [...config.logistics.gatewayProfiles];
       newProfiles[gatewayIndex] = { ...gateway, currencies: newCurrencies };
       onUpdate({ logistics: { ...config.logistics, gatewayProfiles: newProfiles } });
   };
   ...
   return (
      <div className="space-y-4 animate-fadeIn">
         <div className="flex justify-between items-end mb-4">
             <div>
                <h3 className="text-lg font-bold text-slate-800 tracking-tight">Divisas y Métodos de Pago ({gateway.label})</h3>
                <p className="text-xs text-slate-500">Configuración de tasas de cambio específica para {gateway.label}.</p>
             </div>
             <button 
                onClick={() => {
                   ...
                   updateCurrencies([...gateway.currencies, newCurr]);
                   setExpandedIndex(gateway.currencies.length); 
                }}
         ...
         {gateway.currencies.map((curr, idx) => {
                  ...
                  <div className="p-5 flex items-center justify-between cursor-pointer" onClick={() => setExpandedIndex(isExpanded ? null : idx)}>
                     ...
                     <div className="flex items-center gap-4">
                        <div className="text-right">
                            ...
                            <span className="font-mono font-bold text-slate-700">{MockBackend.getClientRate(curr.code, activeGatewaySlug).toFixed(2)}</span>
                        </div>
                     ...
                     <div className="p-6 border-t border-slate-100 bg-slate-50/30 space-y-6 animate-fadeIn">
                        ...
                        <div className="grid md:grid-cols-2 gap-6">
                            <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                                ...
                                <div className="flex bg-slate-100 p-1 rounded-lg">
                                    <button onClick={() => { const nc = [...gateway.currencies]; nc[idx].rateMode = 'MANUAL'; updateCurrencies(nc); }} className={`flex-1 py-1.5 text-xs font-bold rounded transition ${curr.rateMode === 'MANUAL' ? 'bg-white shadow text-slate-800' : 'text-slate-500'}`}>Manual</button>
                                    <button onClick={() => { const nc = [...gateway.currencies]; nc[idx].rateMode = 'DYNAMIC'; updateCurrencies(nc); }} className={`flex-1 py-1.5 text-xs font-bold rounded transition ${curr.rateMode === 'DYNAMIC' ? 'bg-white shadow text-slate-800' : 'text-slate-500'}`}>Dinámica</button>
                                </div>
                            </div>
                            <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                                ...
                                {curr.rateMode === 'MANUAL' ? (
                                    <input type="number" step="0.01" value={curr.manualRate} onChange={e => { const nc = [...gateway.currencies]; nc[idx].manualRate = Number(e.target.value); updateCurrencies(nc); }} className="w-full p-2 bg-slate-50 border border-slate-200 rounded font-mono font-bold text-lg focus:ring-1 focus:ring-brand-500 outline-none" />
                                ) : (
                                    <div className="space-y-3">
                                        <div className="flex bg-slate-100 p-0.5 rounded-lg border border-slate-200">
                                            <button 
                                                onClick={() => { 
                                                    const nc = [...gateway.currencies]; 
                        ...
                        <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
                            <div className="flex justify-between items-center mb-4">
                                <h4 className="text-xs font-bold text-slate-800 uppercase flex items-center gap-2"><CreditCard size={14}/> Métodos para {curr.code}</h4>
                                <button onClick={() => { const nc = [...gateway.currencies]; nc[idx].methods.push({ id: `m-${Date.now()}`, name: 'Nuevo Banco', enabled: true, commission: { pct: 0, fixed: 0, enabled: false } }); updateCurrencies(nc); }} className="text-[10px] font-bold text-brand-600 hover:underline">+ Agregar</button>
                            </div>
                            <div className="space-y-2">
                                {curr.methods.map((method, mIdx) => (
                                    <div key={method.id} className="flex items-center justify-between p-3 bg-slate-50 rounded-lg border border-slate-100 hover:bg-slate-100 transition-colors">
                                        <div className="flex items-center gap-3 flex-1">
                                            ...
                                            <input value={method.name} onChange={e => { const nc = [...gateway.currencies]; nc[idx].methods[mIdx].name = e.target.value; updateCurrencies(nc); }} className="bg-transparent text-sm font-bold text-slate-700 focus:ring-0 border-none p-0 w-2/3" />
                                        </div>
                                        <div className="flex items-center gap-3">
                                            <button onClick={() => { const nc = [...gateway.currencies]; nc[idx].methods[mIdx].enabled = !method.enabled; updateCurrencies(nc); }} className={`px-2 py-1 rounded text-[10px] font-black tracking-tighter transition-all ${method.enabled ? 'bg-green-100 text-green-700' : 'bg-slate-200 text-slate-500'}`}>{method.enabled ? 'ACTIVO' : 'PAUSADO'}</button>
                                            <button onClick={() => { const nc = [...gateway.currencies]; nc[idx].methods = nc[idx].methods.filter((_, i) => i !== mIdx); updateCurrencies(nc); }} className="text-slate-300 hover:text-red-500 transition-colors"><Trash2 size={14}/></button>
                                        </div>
                                    </div>
                                ))}
                            </div>
                        ...
                        <div className="flex justify-end pt-4">
                            <button onClick={() => { const nc = gateway.currencies.filter((_, i) => i !== idx); updateCurrencies(nc); setExpandedIndex(null); }} className="text-xs font-bold text-red-500 hover:underline flex items-center gap-1"><Trash2 size={14}/> Eliminar Divisa Completa</button>
                        </div>
                     </div>
                  )}
               </div>
...
const LogisticsInternalCosts = ({ config, onUpdate, activeGatewaySlug }: { config: SystemConfig, onUpdate: (c: Partial<SystemConfig>) => void, activeGatewaySlug: string }) => {
   const gatewayIndex = config.logistics.gatewayProfiles.findIndex(g => g.slug === activeGatewaySlug);
   const gateway = config.logistics.gatewayProfiles[gatewayIndex];

   if(!gateway) return <div>Seleccione una pasarela</div>;

   const updateCosts = (newCosts: CostGroup[]) => {
       const newProfiles = [...config.logistics.gatewayProfiles];
       newProfiles[gatewayIndex] = { ...gateway, costs: newCosts };
       onUpdate({ logistics: { ...config.logistics, gatewayProfiles: newProfiles } });
   };
   ...
   return (
      <div className="space-y-6 animate-fadeIn">
         <div className="flex justify-between items-end">
             <div>
                <h3 className="text-lg font-bold text-slate-800 tracking-tight">Tarifas y Costos Internos ({gateway.label})</h3>
                <p className="text-xs text-slate-500">Define los gastos operativos específicos para esta pasarela.</p>
             </div>
             <button onClick={() => updateCosts([...gateway.costs, { id: `g-${Date.now()}`, label: 'Nuevo Grupo de Costos', items: [] }])} className="bg-slate-900 text-white px-4 py-2 rounded-lg text-xs font-bold flex items-center gap-2 hover:bg-slate-800"><Plus size={16}/> Nuevo Grupo</button>
         </div>

         {gateway.costs.map((group, gIdx) => (
             <div key={group.id} className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                <div className="bg-slate-50 px-5 py-3 border-b border-slate-200 flex justify-between items-center">
                    <input value={group.label} onChange={e => { const nc = [...gateway.costs]; nc[gIdx].label = e.target.value; updateCosts(nc); }} className="bg-transparent font-bold text-sm text-slate-700 focus:ring-0 border-none p-0 w-2/3" />
                    <button type="button" onClick={() => { if(confirm('¿Eliminar grupo completo?')) updateCosts(gateway.costs.filter(g => g.id !== group.id)); }} className="text-slate-400 hover:text-red-500"><Trash2 size={16}/></button>
                </div>
                    ...
                    {group.items.map((item, iIdx) => (
                        <div key={item.id} className="flex flex-wrap md:flex-nowrap items-center gap-4 bg-slate-50/50 p-3 rounded-lg border border-slate-100">
                           <div className="flex-1 min-w-[150px]">
                               ...
                               <input value={item.label} onChange={e => { const nc = [...gateway.costs]; nc[gIdx].items[iIdx].label = e.target.value; updateCosts(nc); }} className="w-full bg-white border border-slate-200 rounded px-2 py-1 text-xs" />
                           </div>
                           <div className="w-24">
                               <label className="text-[9px] font-bold text-slate-400 uppercase block mb-1">Clasificación</label>
                               <select value={item.category} onChange={e => { const nc = [...gateway.costs]; nc[gIdx].items[iIdx].category = e.target.value as any; updateCosts(nc); }} className="w-full bg-white border border-slate-200 rounded px-2 py-1 text-[10px] font-bold">
                           ...
                           <div className="w-24">
                               ...
                               <input type="number" step="0.01" value={item.value} onChange={e => { const nc = [...gateway.costs]; nc[gIdx].items[iIdx].value = Number(e.target.value); updateCosts(nc); }} className="w-full bg-white border border-slate-200 rounded px-2 py-1 text-xs font-mono" />
                           </div>
                           <div className="w-16">
                               <label className="text-[9px] font-bold text-slate-400 uppercase block mb-1">Tipo</label>
                               <select value={item.type} onChange={e => { const nc = [...gateway.costs]; nc[gIdx].items[iIdx].type = e.target.value as any; updateCosts(nc); }} className="w-full bg-white border border-slate-200 rounded px-2 py-1 text-[10px] font-bold">
                           ...
                           {item.category === 'DEDUCTIVE' && (
                               <div className="flex flex-col items-center justify-center w-24">
                                   ...
                                   <button 
                                      onClick={() => { const nc = [...gateway.costs]; nc[gIdx].items[iIdx].isClientChargeable = !(item.isClientChargeable !== false); updateCosts(nc); }}
                           ...
                           <div className="flex items-center gap-2 pt-4">
                               <button onClick={() => { const nc = [...gateway.costs]; nc[gIdx].items[iIdx].enabled = !item.enabled; updateCosts(nc); }} className={`p-1 rounded ${item.enabled ? 'text-green-600 bg-green-50' : 'text-slate-300 bg-slate-100'}`}><Power size={14}/></button>
                               <button onClick={() => { const nc = [...gateway.costs]; nc[gIdx].items = nc[gIdx].items.filter((_, i) => i !== iIdx); updateCosts(nc); }} className="p-1 text-slate-400 hover:text-red-500"><Trash2 size={14}/></button>
                           </div>
                        </div>
                    ))}
                    <button onClick={() => { const nc = [...gateway.costs]; nc[gIdx].items.push({ id: `i-${Date.now()}`, label: 'Nuevo Gasto', type: 'PCT', category: 'DEDUCTIVE', value: 0, enabled: true, isClientChargeable: true }); updateCosts(nc); }} className="w-full border-2 border-dashed border-slate-200 rounded-lg py-3 text-xs font-bold text-slate-400 hover:border-brand-200 hover:text-brand-600 transition-colors">+ Agregar Item de Costo</button>
                </div>
             </div>
         ))}
      </div>
...
// 3. COUPONS MANAGEMENT (Unchanged logic, just props passed)
  ...
  return (
      <div className="min-h-screen bg-slate-100 flex">
          <aside className={`bg-slate-900 text-white transition-all duration-300 ${isSidebarOpen ? 'w-64' : 'w-20'} flex flex-col fixed h-full z-10 shadow-2xl`}>
               {/* Sidebar content (unchanged mostly) */}
          ...
          <main className={`flex-1 transition-all duration-300 ${isSidebarOpen ? 'ml-64' : 'ml-20'} p-8`}>
               ...
               {currentTab === 'LOGISTICS' && (
                   <div className="animate-fadeIn grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
                       <div className="lg:col-span-2 space-y-6">
                           <SectionHeader title="Logística Financiera" description="Administra la infraestructura de pasarelas, tasas y costos." />
                           
                           {/* TOP LEVEL: GATEWAY SELECTOR */}
                           <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden mb-6">
                                <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                                    <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Pasarelas Generales</h3>
                                    <button 
                                        onClick={() => {
                                            const newGw: GatewayProfile = {
                                                id: `gw-${Date.now()}`, slug: 'NEW', label: 'Nueva Pasarela', iconName: 'Box', enabled: true,
                                                currencies: [], costs: []
                                            };
                                            handleUpdateConfig({ logistics: { ...config.logistics, gatewayProfiles: [...config.logistics.gatewayProfiles, newGw] } });
                                            setActiveGatewaySlug('NEW');
                                        }}
                                        className="text-[10px] bg-brand-600 text-white px-3 py-1.5 rounded-lg font-bold hover:bg-brand-700"
                                    >
                                        + Nueva Pasarela
                                    </button>
                                </div>
                                <div className="flex overflow-x-auto p-2 gap-2 scrollbar-thin">
                                    {config.logistics.gatewayProfiles.map(gw => (
                                        <button
                                            key={gw.id}
                                            onClick={() => setActiveGatewaySlug(gw.slug)}
                                            className={`flex items-center gap-2 px-4 py-3 rounded-xl border transition-all min-w-[140px] ${activeGatewaySlug === gw.slug ? 'bg-brand-50 border-brand-200 text-brand-700 ring-2 ring-brand-100' : 'bg-white border-slate-100 text-slate-500 hover:bg-slate-50'}`}
                                        >
                                            <div className={`w-2 h-2 rounded-full ${gw.enabled ? 'bg-green-500' : 'bg-slate-300'}`}></div>
                                            <span className="font-bold text-sm whitespace-nowrap">{gw.label}</span>
                                            {activeGatewaySlug === gw.slug && (
                                                <div onClick={(e) => {
                                                    e.stopPropagation();
                                                    const newLabel = prompt("Nombre de la pasarela:", gw.label);
                                                    if(newLabel) {
                                                        const profiles = [...config.logistics.gatewayProfiles];
                                                        const idx = profiles.findIndex(p => p.id === gw.id);
                                                        profiles[idx] = { ...profiles[idx], label: newLabel, slug: newLabel.substring(0,6).toUpperCase().replace(/\s/g, '') };
                                                        handleUpdateConfig({ logistics: { ...config.logistics, gatewayProfiles: profiles } });
                                                        setActiveGatewaySlug(profiles[idx].slug);
                                                    }
                                                }} className="ml-auto p-1 hover:bg-brand-100 rounded text-brand-400">
                                                    <Settings2 size={12}/>
                                                </div>
                                            )}
                                        </button>
                           ...
                           <div className="bg-slate-50/50 p-6 rounded-3xl border border-slate-200/50 min-h-[500px]">
                               {logisticsSubTab === 'GATEWAYS' && <LogisticsGateways config={config} onUpdate={handleUpdateConfig} activeGatewaySlug={activeGatewaySlug} />}
                               {logisticsSubTab === 'COSTS' && <LogisticsInternalCosts config={config} onUpdate={handleUpdateConfig} activeGatewaySlug={activeGatewaySlug} />}
                               {logisticsSubTab === 'COUPONS' && <LogisticsCoupons config={config} onUpdate={handleUpdateConfig} />}
                           </div>
                       </div>
                       <div className="lg:col-span-1">
                           <InternalSimulator config={config} activeGatewaySlug={activeGatewaySlug} />
                           ...
                           <div className="mt-6 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                               ...
                               <ul className="space-y-3">
                                   <li className="text-[10px] text-slate-500 border-l-2 border-orange-200 pl-3 py-1">Mantenga el spread operativo por encima del 5% para garantizar rentabilidad tras comisiones de pasarela.</li>
                                   <li className="text-[10px] text-slate-500 border-l-2 border-brand-200 pl-3 py-1">Verifique las tasas dinámicas periódicamente.</li>
                               </ul>
                           </div>
                       </div>
                   </div>
               ...
               {/* OTHER TABS (TRANSACTIONS, MESSAGES, DESIGN) - Same as before */}
